        ...
        while (true) {
            ...
            if (_access(dir.data(), 0) == -1) { //access函数是查看是不是存在
                if (mkdir(dir.data(), mod) == -1) {  //如果不存在就用mkdir函数来创建
                    ...
                    WarnL << "mkdir " << dir << " failed: " << dir.c_str();
                    return false;
                }
            }
        }
...
//Determine if it is a directory
    ...
    int getontimeofday(struct timeval* tp, void* tzp) {
...
void AsyncLogWriter::flushAll() {
    ...
    for (auto& it : tmp) {
        it.second->writeChannels(it.first);
    }
...
void SysLogChannel::write(const Logger &logger, const LogContextPtr &ctx) {
    ...
    static bool sys_log_init = false;
    if(!sys_log_init){
        sys_log_init = true;
...
FileChannel::FileChannel(const string &dir, const string& name, LogLevel level) : FileChannelBase(name, "", level) {
    _dir = dir;
    if (_dir.back() != '/') {
        _dir.append("/");
    }
    ...
    //获取今天日志文件的最大index号
    auto log_name_prefix = getTimeStr("%Y-%m-%d_",0);
