...
namespace download {
	...
	void URLDownloadTask::startDownload(DownloadStatusNotifyCB downloadCB)
	{
		...
		auto task = [=] {
			...
			bool ret = httpUrlSplit::get_addr(url.c_str(), host, url_path, sizeof(host), &port);
			if (!ret) {
				return;
			}
			...
			auto res = _http_client->Get(url_path,
				...
				[=, &total_size](uint64_t len, uint64_t total) {
					...
					total_size = total+ resume_size;
					downloadCB(downloadStatus::DOWNLOAD_PROGRESS, len, total_size, _downInfo, _download_path);
					return true;
				});
			...
			if (download_size+ resume_size == total_size && total_size != 0) {
				if (_downInfo.support_resume) {
					std::remove(resume_file.c_str());
				}
				int ret = std::rename(tmp_part.c_str(), _download_path.c_str());
				if (ret == 0) {
					downloadCB(downloadStatus::DOWNLOAD_SUCCESS, download_size, total_size, _downInfo, _download_path);
				}
				else {
					std::remove(tmp_part.c_str());
				}
