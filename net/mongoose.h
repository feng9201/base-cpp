...
// add restrict_s support is possible
#if (defined(_MSC_VER) && _MSC_VER >= 1900) || defined(__clang__) || \
    defined(__GNUC__)
#define restrict_s __restrict
#else
#define restrict_s
#endif
#endif
...
PORTABLE_8439_DECL size_t mg_chacha20_poly1305_encrypt(
    uint8_t *restrict_s cipher_text, const uint8_t key[RFC_8439_KEY_SIZE],
    const uint8_t nonce[RFC_8439_NONCE_SIZE], const uint8_t *restrict_s ad,
    size_t ad_size, const uint8_t *restrict_s plain_text, size_t plain_text_size);
...
PORTABLE_8439_DECL size_t mg_chacha20_poly1305_decrypt(
    uint8_t *restrict_s plain_text, const uint8_t key[RFC_8439_KEY_SIZE],
    const uint8_t nonce[RFC_8439_NONCE_SIZE],
    const uint8_t *restrict_s cipher_text, size_t cipher_text_size);
#if defined(__cplusplus)
}
#endif
#endif
